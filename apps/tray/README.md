...
# Icons are generated, never committed — and `generate_context!` reads them at
# compile time, so a fresh clone fails to build until this is run once.
node ../../scripts/make-icon.mjs app-icon.png 1024 && npx tauri icon app-icon.png

npm run dev
```

Nothing binary is committed: `src-tauri/icons/` and `app-icon.png` are both
gitignored and regenerated by the step above (and by CI).
